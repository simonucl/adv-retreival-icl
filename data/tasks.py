...
if __name__ == '__main__':
    origin_path='.../datasets_processed'
    data_path='.../perfect_data'
    for seed in [100, 13, 21, 42, 87]:
        for task_name in ["mr", "cr", "subj", "SST-2", "trec", "sst-5",
                          "rte", "cb", "wic", "qnli", "qqp", "mrpc"]:
            task = AutoTask.get(
                task=task_name, 
                data_seed=seed,
                num_samples=16,
                cache_dir=None, 
                data_dir=origin_path)

            datasets = task.get_datasets()

            save_path = join(data_path, task_name, '16-{0}'.format(str(seed)))
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            datasets["train"].to_json(join(save_path, 'train.json'))
            datasets["validation"].to_json(join(save_path, 'valid.json'))
            datasets["test"].to_json(join(save_path, 'test.json'))